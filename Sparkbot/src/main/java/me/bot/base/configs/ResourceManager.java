...
public class ResourceManager {
	...
	public HashMap<String, Object> getConfig(String dir, String filename) throws JsonProcessingException {
	...
	public void saveAll() {
		...
		System.out.println("Called " + configMapper);
	...
	private void saveConfig(File file, HashMap<String, Object> config) {
		...
		if (!file.exists()) {
			try {
				...
				logger.error("Unexpected exception when creating config file", e);
			}
		}
		...
		try {
			...
			logger.error("Unexpected exception when writing config file", e);
		}
	}
	...
	public Language loadLanguage(String identifier) {
		...
		if (!file.exists()) {
			try {
				Language lang = languageClass.getDeclaredConstructor().newInstance();
				writeFile(mapper.writeValueAsString(lang), file);
			} catch (InstantiationException | IllegalAccessException | JsonProcessingException | NoSuchMethodException |
					 InvocationTargetException e) {
				logger.error("Unexpected exception when creating new language file", e);
			}
		} else {
			...
			try {
				...
				logger.error("Unexpected exception when loading language file", e);
			}
			
		}
		try {
			return languageClass.getDeclaredConstructor()
								.newInstance();
