...
public class Macro implements ICommand {
            ...
            switch (args[0]) {
                case "format":
                    if (message.getAttachments().isEmpty() && args.length > 1) {
                        ...
                        FormatObject result = MacroFormatter.format(new ArrayList<>(Arrays.asList(code)), flags[0], flags[1], flags[3]);
                        printObject(channel, result, "temp.txt", flags[2], flags[4]);
                        break;
                    }
    ...
    private boolean format(TextChannel channel, String message, Attachment attachment) {
        ...
        if (attachment.getFilename().endsWith("txt")) {
            try {
                ...
                FormatObject object = MacroFormatter.format(file, flags[0], flags[1], flags[3]);
                printObject(channel, object, attachment.getFilename(), flags[2], flags[3]);
            } catch (Exception e) {
                e.printStackTrace();
                return false;
            }
    ...
    private static String[] toCheck = {"full","diff","debug","caps","file"};
        ...
        return spec -> {
            ...
            if (struct.category != null && struct.type != null) {
                category = category
                        ...
                        .replace("$CATURL$", "http://beta.mkb.gorlem.ml/docs/" + struct.type.getUrl() + "#" + struct.category.replace(" ", "%20"));
                desc.append(" | ");
            ...
            if (struct.version != null) {
                spec.setFooter("Since Macromod " + struct.version.toString() + " | Minecraft " + struct.version.minecraft, "https://spthiel.github.io/files/704612_gear_512x512.png");
            }
            spec.setAuthor("Macromod", null, "https://cdn.discordapp.com/icons/300295653907628032/0450d7603dff67d64640780b97c2bbc7.webp");
            ...
            if (struct.related != null && struct.related.size() > 0) {
                ...
                for (ResponseStruct.Related rel : struct.related) {
                    String toAppend = "[" + rel.name + "](http://beta.mkb.gorlem.ml/" + rel.resource.replace("/api/", "") + ")\n";
